import { filter, map, mutate, pipe, } from '../';
...
export function statechart(config, chartDefinition) {
    let currentInstance;
    ...
    return {
        ...
        actions: Object.keys(actions).reduce((aggr, key) => {
            aggr[key] = pipe(map(function getTransition({ state, execution }, payload) {
            ...
            }), mutate(function runAction(context, { payload }) {
                const stateTarget = getTarget(context.state, context.execution.namespacePath);
                const actionsTarget = getTarget(context.actions, context.execution.namespacePath);
